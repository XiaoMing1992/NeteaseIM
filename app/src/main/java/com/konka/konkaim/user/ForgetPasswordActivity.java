...
/*                if (s.length() == 11)
    ...
    private void sendVerifyCode() {
        ...
        HttpHelper.sendSmsCode(mobileStr, Constant.RESETPSW_TYPE);
    }
    ...
    private void resetPassword() {
        ...
        HttpHelper.resetPassword(mobileStr, passwordStr, smsCodeStr);
    }
    ...
    private HttpListener<BaseBean> httpListener = new HttpListener<BaseBean>() {
        ...
        public void fail(Throwable e, String type) {
            if (type.equals(HttpHelper.SMS_TYPE)) {
                showToast(ForgetPasswordActivity.this, "发送短信失败");
            } else if (type.equals(HttpHelper.IS_EXIST_TYPE)) {
                showToast(ForgetPasswordActivity.this, "检查手机是否已经注册失败");
            } else if (type.equals(HttpHelper.RESET_PASSWORD_TYPE)) {
                showToast(ForgetPasswordActivity.this, "重置密码失败");
            }
        }
            ...
            if (type.equals(HttpHelper.SMS_TYPE)) {
                ...
                if (baseBean != null) {
                    if (baseBean.getCode().equals(Constant.USER_NOT_EXIST)) {
                        ...
                        showToast(ForgetPasswordActivity.this, "验证码已发");
                    }
                }
            } else if (type.equals(HttpHelper.RESET_PASSWORD_TYPE)) {
                if (baseBean != null) {
                    if (baseBean.getCode().equals(Constant.USER_NOT_EXIST)) {
                    ...
                    } else if (baseBean.getCode().equals(Constant.RESET_PASSWORD_FAIL)) {
                        mobile_error_tip.setVisibility(View.GONE);
                        verifyCode_error_tip.setVisibility(View.GONE);
                        password_error_tip.setVisibility(View.GONE);
                        showToast(ForgetPasswordActivity.this, "重置密码失败");
                        ...
                        verifyCode_error_tip.setVisibility(View.GONE);
                        password_error_tip.setVisibility(View.GONE);
                        showToast(ForgetPasswordActivity.this, "重置密码成功");
        ...
        switch (v.getId()) {
            case R.id.send_verifyCode:
                if (!NetworkUtil.isNetworkAvailable(ForgetPasswordActivity.this)) {
                    showToast(ForgetPasswordActivity.this, "当前网络不可用，请检查网络是否连接");
                    return;
                }
