...
//        }
    ...
    public void doCalling(String account, final AVChatType avChatType, final AVChatControllerCallback<AVChatData> callback) {
        AVChatNotifyOption notifyOption = new AVChatNotifyOption();     //包含iOS的通知配置, WebRTC兼容以及可自定义的扩展消息
        notifyOption.forceKeepCalling = false;                          //默认true->离线持续呼叫功能
        notifyOption.apnsBadge = false;
        notifyOption.apnsContent = "通话请求";
        notifyOption.pushSound = "video_chat_push.mp3";
...
//                DialogMaker.dismissProgressDialog();
            ...
            public void onFailed(int code) {
                LogUtil.LogD(TAG, "avChat call failed code->" + code);

                if (code == ResponseCode.RES_FORBIDDEN) { //应用被封禁
//                    Toast.makeText(context, "应用被封禁", Toast.LENGTH_SHORT).show();
                    //Toast.makeText(context, R.string.avchat_no_permission, Toast.LENGTH_SHORT).show();
                } else {
//                    Toast.makeText(context, "avChat call failed code->" + code, Toast.LENGTH_SHORT).show();
            ...
            public void onException(Throwable exception) {
                LogUtil.LogD(TAG, "avChat call onException->" + exception);
                closeRtc(avChatType == AVChatType.VIDEO ? CallStateEnum.VIDEO : CallStateEnum.AUDIO);
...
//        AVChatManager.getInstance().setParameter(AVChatParameters.KEY_VIDEO_FRAME_FILTER, true);
        AVChatManager.getInstance().accept2(avChatData.getChatId(), new AVChatCallback<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                LogUtil.LogI(TAG, "accept success");
...
//                    Toast.makeText(context, "建立连接失败", Toast.LENGTH_SHORT).show();
                ...
                LogUtil.LogE(TAG, "accept onFailed->" + code);
            ...
            @Override
            public void onException(Throwable exception) {
                LogUtil.LogD(TAG, "accept exception->" + exception);
                handleAcceptFailed(avChatType == AVChatType.VIDEO ?
                        CallStateEnum.VIDEO_CONNECTING : CallStateEnum.AUDIO);
                callback.onFailed(-1, exception.toString());
            }
    ...
    public void switchVideoToAudio(final AVSwitchListener avSwitchListener) {
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_VIDEO_TO_AUDIO, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtil.LogD(TAG, "videoSwitchAudio onSuccess");
                //关闭视频
            ...
            @Override
            public void onFailed(int code) {
                LogUtil.LogD(TAG, "videoSwitchAudio onFailed");
            }

            @Override
            public void onException(Throwable exception) {
                LogUtil.LogD(TAG, "videoSwitchAudio onException");
            }
        });
    }
    ...
    public void switchAudioToVideo(final AVSwitchListener avSwitchListener) {
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_AUDIO_TO_VIDEO, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtil.LogD(TAG, "requestSwitchToVideo onSuccess");
                avSwitchListener.onAudioToVideo();
            }

            @Override
            public void onFailed(int code) {
                LogUtil.LogD(TAG, "requestSwitchToVideo onFailed" + code);
            }

            @Override
            public void onException(Throwable exception) {
                LogUtil.LogD(TAG, "requestSwitchToVideo onException" + exception);
            }
        });
    }
    ...
    public void receiveAudioToVideo(final AVSwitchListener avSwitchListener) {
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_AUDIO_TO_VIDEO_AGREE, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtil.LogD(TAG, "receiveAudioToVideo onSuccess");
            ...
            @Override
            public void onFailed(int code) {
                LogUtil.LogD(TAG, "receiveAudioToVideo onFailed");
            }

            @Override
            public void onException(Throwable exception) {
                LogUtil.LogD(TAG, "receiveAudioToVideo onException");
            }
        });
    }
    ...
    public void hangUp(int type) {
        ...
        if ((type == AVChatExitCode.HANGUP || type == AVChatExitCode.PEER_NO_RESPONSE
            ...
            AVChatManager.getInstance().hangUp2(avChatData.getChatId(), new AVChatCallback<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    System.out.println("hangup onSuccess");
                ...
                @Override
                public void onFailed(int code) {
                    LogUtil.LogD(TAG, "hangup onFailed->" + code);
                    System.out.println("hangup onFailed->" + code);
//                    Toast.makeText(context, "断开通话失败", Toast.LENGTH_SHORT).show();
                    //showQuitToast();
                    //((Activity)context).finish();
                }
                ...
                @Override
                public void onException(Throwable exception) {
                    LogUtil.LogD(TAG, "hangup onException->" + exception);
                    System.out.println("hangup onException");
                    exception.printStackTrace();
//                    Toast.makeText(context, "断开通话异常", Toast.LENGTH_SHORT).show();
                    //((Activity)context).finish();
                }
            });
        }
        ...
        switch (code) {
            ...
            case AVChatExitCode.HANGUP:
                ...
                if (isCallEstablish.get()) {
                    //Toast.makeText(context, "通话完成", Toast.LENGTH_SHORT).show();
