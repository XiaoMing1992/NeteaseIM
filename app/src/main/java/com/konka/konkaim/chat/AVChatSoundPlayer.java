...
public class AVChatSoundPlayer {
    ...
    public enum RingerTypeEnum {
        CONNECTING,        //发出来电铃声                  (拨打)
        NO_RESPONSE,       //对方无人接听，请稍后再拨       (超时自动挂断)
        PEER_BUSY,         //对方正在通话中，请稍后再拨    （不使用该种铃声，因为无法区分 通话中和忙碌）
        PEER_REJECT,       //对方暂时无法接听，请稍后再拨   (忙碌 / 通话中 /拒绝)
        RING,;             //收到来电铃声                  (来电)
    }
    ...
    public synchronized void play(RingerTypeEnum type) {
        ...
        int ringId = 0;
        switch (type) {
            case NO_RESPONSE://对方无人接听，请稍后再拨
                ringId = R.raw.video_chat_tip_ontimer;
                loop = false;
                break;
            case PEER_BUSY://对方正在通话中，请稍后再拨
                ringId = R.raw.video_chat_tip_oncall;
                loop = false;
                break;
            case PEER_REJECT://对方暂时无法接听，请稍后再拨
                ringId = R.raw.video_chat_tip_hangup;
                loop = false;
                break;
            case CONNECTING:
                ringId = R.raw.video_chat_tip_sender;
                loop = true;
                break;
            case RING:
                ringId = R.raw.video_chat_tip_receiver;
                loop = true;
                break;
        }
